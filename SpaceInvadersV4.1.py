...
def deplacement():
    global X1,Y1,X2,Y2,dX,dY,Largeur,Hauteur
    TailleAlien=50
    #rebond à gauche
    if X1+dX<0:
        X1=0
        ...
        Y2+=TailleAlien
    #rebond à droite
    if X2+TailleAlien/2+dX>Largeur:
        X2=Largeur-TailleAlien
        dX=-dX
        Y2+=TailleAlien
        Y1+=TailleAlien
    X1=X1+dX
    ...
    Y2=Y2+dY
    #affichage
    Canevas.coords(Alien1,X1,Y1,X1+TailleAlien,Y1+TailleAlien)
    Canevas.coords(Alien2,X2,Y2,X2+TailleAlien,Y2+TailleAlien)
    #déplacement
